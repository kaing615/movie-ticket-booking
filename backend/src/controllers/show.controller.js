...
export const getSeatsOfShow = async (req, res) => {
	try {
		...
		if (!show)
			return responseHandler.notFound(res, "Không tìm thấy suất chiếu.");
...
export const holdSeats = async (req, res) => {
	try {
		...
		// Ghế đã sold?
		const [reservedIds, ticketIds] = await Promise.all([
			SeatReservation.find({ showId, seatId: { $in: seatIds } }).distinct(
				"seatId"
			),
			Ticket.find({
				showId,
		...
		if (conflictHolds > 0) {
			return responseHandler.conflict(
				res,
				"Một số ghế đang được người khác giữ."
			);
		}
		...
		return responseHandler.ok(res, {
			...
			hold: {
				seatIds: hold.seatIds.map(String),
...
export const refreshHold = async (req, res) => {
	try {
		...
		if (!hold)
			return responseHandler.notFound(res, "Không có hold để gia hạn.");
...
export const releaseHold = async (req, res) => {
	try {
		...
		if (Array.isArray(seatIds) && seatIds.length > 0) {
			const keep = hold.seatIds.filter(
				(id) => !seatIds.map(String).includes(String(id))
			);
