...
const TheaterInfo = () => {
  ...
  const { data: shows } = useQuery({
    ...
    queryFn: () => showApi.getShowtimesByTheater(selectedTheaterId),
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8">
        ...
        {theaterSystems?.map((system) => (
          <button
            ...
            className={`flex items-center justify-center rounded-lg w-14 h-14 p-2 border 
              ${selectedSystemId === system._id ? "bg-gray-100 border-[#034ea2]" : "border-gray-200"}
          ...
          {theaters.map((theater) => (
            <button
              ...
              className={`px-3 py-2 rounded border 
                ${selectedTheaterId === theater._id ? "bg-blue-50 border-[#034ea2]" : "border-gray-200"}
                hover:bg-gray-100 transition`}
              onClick={() => handleSelectTheater(theater._id)}
            >
              <span className="font-semibold">{theater.theaterName}</span>
              <span className="block text-xs text-gray-500">{theater.address}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="bg-white rounded-lg border p-4">
        <h3 className="text-sm font-semibold mb-3 text-gray-700">Lịch chiếu phim</h3>
          ...
          <div className="space-y-3 max-h-[450px] overflow-y-auto">
            {shows.map((show) => (
              <div
                ...
                onClick={() => navigate(`/movie-details/${show.movieId}`)}
              >
                <img
                  src={show.poster}
                  alt={show.movieName}
                ...
                <div className="flex-1">
                  <h4 className="text-base font-semibold">{show.movieName}</h4>
                  <div className="flex flex-wrap gap-2 mt-1">
                    {show.times?.slice(0, 4).map((item, idx) => (
                      <span key={idx} className="text-xs bg-blue-50 px-2 py-1 rounded text-blue-600">
                        {new Date(item).toLocaleString("vi-VN")}
                      </span>
                    ))}
                    {show.times?.length > 4 && (
                      <span className="text-xs text-gray-400">+{show.times.length - 4} suất nữa</span>
                    )}
                  </div>
                </div>
              </div>
